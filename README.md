...
> **选错文件夹不会静默失败**：面板每次保存前都会确认"这个文件夹里真的有这个文件"（`getFileHandle` 不带 `create`），
> 没有就拒绝写入、清掉这次授权并提示重选 —— 避免出现"提示保存成功、其实写到了「下载」"这种坑。
> 保存成功的提示会带上文件夹名（`已保存到文件夹「xxx」`），写完还会 `getFile().size` 比对一次确认落盘。
