...
| 3 | 都没有（本机第一次） | 弹**一次**"另存为"，且**目录已定位到原文件夹、文件名已预填** |

> **第一次一定要授权一次，绕不过去**：浏览器不允许网页未经允许写磁盘，而且磁盘路径字符串无法变成可写句柄 —— 这是浏览器的安全模型，任何方案都做不到"完全不授权就写文件"。
>
> 好在可以把成本降到最低：点一次右下角 **「📁 授权文件夹（一次）」**（之后整个文件夹永久免弹窗），或者把当前 HTML **直接拖进页面**（零弹窗）。
</details>
